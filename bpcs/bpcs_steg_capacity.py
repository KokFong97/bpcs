...
def histogram_of_complexity(arr, grid_size, alpha, comp_fcn):
    ...
    vals = [arr_bpcs_complexity(arr[tuple(dims)])
...
def flip_image_complexity(arr, alpha, comp_fcn, grid_size):
    ...
    for dims in get_next_grid_dims(arr, grid_size):
        ...
        if comp_fcn(arr_bpcs_complexity(grid), alpha):  # < or >
            ...
            assert abs((1 - arr_bpcs_complexity(init_grid)) -
                       arr_bpcs_complexity(grid)) < 0.01
            assert not(arr[dims].tolist() ==
                       init_grid.tolist() and alpha != 0.5)
    log.critical('Conjugated {0} grids'.format(n))
    # histogram_of_complexity(arr, params)
    return arr, n
...
class HistogramComplexityImage(ActOnImage):
    def modify(self, alpha, comp_fcn, grid_size=(8, 8)):
        hist, navail, ntotal = histogram_of_complexity(
            self.arr, grid_size, alpha, comp_fcn)
        log.critical('{0} of {1} grids available with alpha of {2}'.format(
            navail, ntotal, alpha))
        nbits_per_grid = grid_size[0]*grid_size[1]
        nbytes = (get_n_message_grids(
            [nbits_per_grid]*int(navail), navail)*nbits_per_grid)/8.0
        percent = nbytes*1.0/os.path.getsize(self.infile)
        log.critical(
            'Approximately {0} bytes of storage space can fit in this vessel image.'.format(nbytes))
        log.critical('{0} byte message would utilize {1:.1%} of the vessel image.'.format(
            nbytes, percent))
        return hist
...
def histogram(infile, outfile, alpha, comp_fcn):
    x = HistogramComplexityImage(
        infile, as_rgb=True, bitplane=True, gray=True, nbits_per_layer=8)
    hist = x.modify(alpha, comp_fcn)
    if outfile is not None:
        hist.savefig(outfile)
        log.critical(
            'Wrote histogram of image complexity to {0}'.format(outfile))
        # plt.show()


def complexify(infile, outfile, alpha):
    x = ComplexifyImage(infile, as_rgb=True, bitplane=True,
                        gray=True, nbits_per_layer=8)
...
def simplify(infile, outfile, alpha):
    x = SimplifyImage(infile, as_rgb=True, bitplane=True,
                      gray=True, nbits_per_layer=8)
    arr, stats = x.modify(alpha)
    x.write(outfile, arr)
    return stats
...
def capacity(infile, alpha=0.45, outfile=None):
    def greater(x, thresh): return x >= thresh
