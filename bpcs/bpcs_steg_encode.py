...
def get_message_and_status(message, dims, conjugated, status, alpha):
    ...
    if status == DEAD:
        ...
        cur, conjugated = get_next_message_grid_sized(
            np.array(conjugated), dims, min_alpha=alpha)
...
def embed_message_in_vessel(arr, alpha, message, grid_size):
    ...
    for dims in get_next_grid_dims(arr, grid_size):
        ...
        if arr_bpcs_complexity(grid) < alpha:
            nleft += 1
            continue
        cur_message, message, conjugated, status = get_message_and_status(
            message, grid.shape, conjugated, status, alpha)
        if status == DEAD:
            ...
            if not arr_bpcs_complexity(cur_message) < alpha:
                ...
                raise Exception(
                    'Error fixing vessel grid to have complexity below alpha: {0} => {1}'.format(a, b))
            nleft += 1
        if status == ALIVE and arr_bpcs_complexity(cur_message) < alpha:
            ...
            if not arr_bpcs_complexity(cur_message) >= alpha:
                ...
                b = arr_bpcs_complexity(cur_message)
                raise Exception(
                    'Error fixing message grid to have complexity above alpha: {0} => {1}'.format(a, b))
            nmessgs += 1
    ...
    if message is not None and message.size > 0:
        raise Exception(
            "Could not fit message in arr. Still had {0} bits left".format(message.size))
    elif status != DEAD:
        raise Exception("Could not fully embed conjugation head in arr.")
    nfound = nmessgs + nmaps + nleft
    assert nmessgs + nmaps + \
        nleft == ngrids, '{0} + {1} + {2} = {4} != {3}'.format(
            nmessgs, nmaps, nleft, ngrids, nfound)
    log.critical(
        'Embedded {0} message grids and {1} conjugation maps'.format(nmessgs, nmaps))
    return arr
...
def encode(infile, messagefile, outfile, alpha=0.45):
    x = BPCSEncodeImage(infile, as_rgb=True, bitplane=True,
