...
def get_n_message_grids(nbits_per_map, ngrids):
    ...
    """
    x, y = ngrids-1, 1
    def is_valid(x, y, ngrids, nbits_per_map): return ngrids == x + \
        y and sum(nbits_per_map[-(y-1):]) < x <= sum(nbits_per_map[-y:])
...
def separate_conj_map_from_message(grids, alpha):
    ...
    def get_nbits_per_map(
        grid): return grid.shape[0]*grid.shape[1] - get_nignored(grid)
